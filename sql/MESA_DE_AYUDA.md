...
ahí, y los ejercicios del Día 5 quedan guardados en la sesión del Día 4. En la app cada
pestaña muestra **solo su propio paquete**, así que esos registros no se pueden editar
desde el día en que aparecen — hay que moverlos a la sesión que les corresponde.

`sql/MOVER_REGISTROS.sql` hace justamente eso. Tiene tres pasos: el PASO 0 lista las
sesiones de esa fecha y entrega el id, se pega ese id en las dos líneas marcadas del
bloque, y el PASO 2 muestra cómo quedó. Mueve solo los registros cuyo ejercicio **no
está** en el plan del día de origen pero **sí está** en el del destino; lo demás no lo
toca. Si el ejercicio no está en ninguno de los dos planes, primero hay que agregarlo al
del destino.

Para encontrar todos los casos de una vez:

```sql
select p.email, ws.fecha, d.orden as dia_n, e.nombre as ejercicio
  from set_logs sl
  join workout_sessions ws on ws.id = sl.session_id
  join profiles p          on p.id = ws.user_id
  join routine_days d      on d.id = ws.routine_day_id
  join exercises e         on e.id = sl.exercise_id
  left join routine_exercises re on re.routine_day_id = ws.routine_day_id
       and re.exercise_id = sl.exercise_id and re.activo
 where re.id is null
 order by ws.fecha desc;
```
